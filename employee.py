...
from functions import size_changer, go_back, clear_content,connect_database, treeview_data,delete
    ...
    if (empid=='' or name =='' or email == '' or gender == 'select gender' or contact =='' or empType == 'Employment Type' or education == 'Education' or workshift == 'Work Shift' or address == '\n' or salary == '' or userType == 'User Type' or password == ''):
        ...
        try:
            ...
            if not cursor or not connection :
                ...
                cursor.execute('SELECT empid frq2om employee_data WHERE empid=%s',(empid,))
                if cursor.fetchone():
                    messagebox.showerror('Error', 'Id already Exists')
                    return
                address=address.strip()
                cursor.execute('INSERT INTO employee_data VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', (empid,name, email, gender, dob, contact, empType, education, workshift,address, doj, salary, userType, password))  
...
def update_employee(empid,name, email, gender, dob, contact, empType, education, workshift,address, doj, salary, userType, password):
    ...
    if not selected:
        ...
        cursor, connction = connect_database()
        if not cursor or not  connction:
            return
        try:
            cursor.execute('use invetory_system')
            ...
            address=address.strip()
            new_data =(name, email, gender, dob, contact, empType, education, workshift,address, doj, salary, userType, password)
            if current_data==new_data:
                messagebox.showinfo('Informaton', 'No changes deteted')
                return
       

            cursor.execute('UPDATE employee_data set name = %s, email=%s, gender = %s, dob=%s, contact= %s, employement_type=%s, education=%s, work_shift = %s, address = %s, doj=%s, salary=%s, usertype=%s, password=%s WHERE empid=%s',(name, email, gender, dob, contact, empType, education, workshift,address, doj, salary, userType, password,empid,)) 
