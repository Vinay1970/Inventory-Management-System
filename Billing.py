...
import math
from functions import size_changer, go_back, clear_content,connect_database, treeview_data, delete, calculator
...
def create_bill_pdf(billNo):
    ...
    if result is None:
        ...
        grtotal = round(result,2)
 
    cursor.execute("SELECT SUM(discountamt) FROM sale_data WHERE billNo = %s", (billNo,))
    discountResult = cursor.fetchone()[0]

    if discountResult is None:
        discountAmount = 0
    else:
        discountAmount = round(discountResult,2)
 
    net_payable = grtotal-discountAmount
...
def show_sale_data(saleTreeview, billNo, lbl_amt,lbl_disc,lbl_net_pay):
    ...
    saleTreeview.delete(*saleTreeview.get_children())
    cursor, connection = connect_database()
    # Query only rows with the same billNo
...
def show_bill_data(cartTreeview, billNo, cursor):
    # Clear existing rows in Treeview
...
def addProduct(client, phno, crdate, billno,name, price, quantity, Treeview, cartTitle, total, discount):
    ...
    """)
   # Check total quantity for this billNo
    ...
    show_bill_data(Treeview,billno,cursor)
...
def search_product(search_entry,treeview):
  if search_entry=='':
    ...
    if not cursor or not connection:
      return
    cursor.execute('USE invetory_system')
    cursor.execute(f"SELECT * FROM product_data WHERE name LIKE = '{search_entry}'")
...
# cart Buttons frame
  ...
  btnAddPrd = Button(buttonsFrame, 
                   ...
                   font=('gody old style', 10,'bold'), 
                   bg='#262626', 
                   fg='lightyellow', command= lambda: addProduct(nameEntery.get(), phoneEntery.get, date.today(), billNo.get(),prdEntry.get(), priceEntry.get(), qtyEntry.get(), cartTreeview, cartTitle, total, discount.get()))
