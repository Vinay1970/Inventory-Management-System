...
def tax_window():
    def save_tax():
        ...
        cursor.execute("""CREATE TABLE IF NOT EXISTS tax_lable (id INT primary key AUTO_INCREMENT, tax DECIMAl(5,2))""")
        cursor.execute('INSERT INTO tax_Lable(tax) VALUES(%s)',value)
        connection.commit()
        messagebox.showinfo('Success', f'New {value} Tax is saved Successfully', parent=tax_root)
...
def update_time():
    ...
    total_empno_lable.config(text=len(records))

    cursor.execute('SELECT * from supplier_data')
    records = cursor.fetchall()
    total_supno_lable.config(text=len(records))

    cursor.execute('SELECT * from category_data')
    records = cursor.fetchall()
    total_catno_lable.config(text=len(records))

    cursor.execute('SELECT * from product_data')
    records = cursor.fetchall()
    total_pdcno_lable.config(text=len(records))
